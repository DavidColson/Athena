...
namespace An::FileSys
{
    ...
    bool FileSys::NewDirectories(const Path& newPath)
    {
        ...
        for (Path m_path : newPath.RelativePath())  
        {
            cumulator /= m_path;
            if (!Exists(m_path))
            {
    ...
    eastl::vector<Path> FileSys::ListFiles(const Path& path)
    {
        ...
        for (Path m_path : FileSys::DirectoryIterator(path))
        {
    ...
    {
        m_directoryToIterate = directory;;
    }
    ...
    {
        if (IsEmpty(m_directoryToIterate))
            return end();

        WIN32_FIND_DATAA data;
        HANDLE hFile = FindFirstFileA((m_directoryToIterate / "*").AsRawString(), &data);

        while (!strcmp(data.cFileName, ".") || !strcmp(data.cFileName, ".."))
            FindNextFileA(hFile, &data);

        return Iterator((void*)hFile, Path(data.cFileName), m_directoryToIterate);
    }
    ...
    {
        return Iterator((void*)INVALID_HANDLE_VALUE, Path(), m_directoryToIterate);
    }
    ...
    FileSys::RecursiveDirectoryIterator::Iterator& FileSys::RecursiveDirectoryIterator::Iterator::operator++()
    {
        ...
        if (!FindNextFileA((HANDLE)currentHandle, &data))
        {
            ...
            if (!pathStack.empty())
            {
                ...
                // We've already searched the m_path at the top of the stack, no need to search it again on the next round so we skip it here
