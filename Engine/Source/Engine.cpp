...
{
	{
		...
		An::AssetHandle monkey("Game/Assets/Spitfire.gltf:mesh_13");
		...
		while (gameRunning)
		{
			...
			Matrixf camera = Matrixf::MakeTranslation(Vec3f(0.0f, 0.0f, 8.0f));
			...
			An::Mesh* pMonkeyRaw = An::AssetDB::GetAsset<An::Mesh>(monkey);
			An::Primitive* pMonkeyPrim = &(pMonkeyRaw->m_primitives[0]);

			bgfx::setVertexBuffer(0, pMonkeyPrim->m_vertexBuffer);
			bgfx::setIndexBuffer(pMonkeyPrim->m_indexBuffer);
